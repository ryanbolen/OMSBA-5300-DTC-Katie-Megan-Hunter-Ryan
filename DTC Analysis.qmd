...
```{r}
main_data <- read_dta("IPUMS.dta.gz")
...
industry_sorted_data <- industry_sorted_data %>% mutate(industry_f = replace_na(industry_name,'other'))

# Summarize the number of observations in our sample that are in each industry sector in each year 

obs_per_year <- industry_sorted_data %>% select(year,cpsid,ind,industry_name,industry_f)

obs_per_year <- obs_per_year %>% group_by(year,industry_f) %>% summarise(tot = n())
